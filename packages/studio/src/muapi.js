    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            if (status === 'completed' || status === 'succeeded' || status === 'success') {
                const outputUrl = data.data?.url || data.outputs?.[0] || data.url || data.output?.url;
...
async function submitAndPoll(endpoint, payload, key, onRequestId, maxAttempts = 60) {
    // ppq.ai uses POST /v1/videos with { model, prompt, ... }
    const body = { ...payload };
    if (!body.model) body.model = endpoint;
    const url = `${BASE_URL}/v1/videos`;
    ...
    const outputUrl = result.outputs?.[0] || result.data?.url || result.url || result.output?.url;
